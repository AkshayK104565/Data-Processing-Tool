"""
Pattern Data Processing Automation Tool — Python Backend
=========================================================
...
# ── CORS: allow every origin, every method ───────────────────
CORS(app,
     origins="*",
...
# ── Manually add CORS headers to EVERY response (belt + braces)
@app.after_request
...
# ── ROUTE 2: Batch URL → filename ───────────────────────────
@app.route("/api/resolve-filenames", methods=["POST", "OPTIONS"])
...
# ── ROUTE 3: Full Image Stack processing ────────────────────
@app.route("/api/process-image-stack", methods=["POST", "OPTIONS"])
def api_process_image_stack():
    ...
    try:
        wb_in = openpyxl.load_workbook(
            io.BytesIO(request.files["file"].read()), data_only=True
        )
    ...
    HDR    = ["Import Type", "Collection Folder", "Image Stack Group",
              "Filename", "Image Stack Order"]
